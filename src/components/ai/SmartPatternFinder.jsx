...
import { Brain, Search, Hash, Zap, Loader2, Sparkles, Download } from "lucide-react";
      ...
      <div className={`relative z-20 max-w-7xl w-full mx-auto flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 sm:gap-6 lg:gap-8 mb-6 sm:mb-8 ${!data && !loading ? "px-4 pt-14 pb-6 sm:p-8 lg:p-10" : ""}`}>
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] text-indigo-400">Analysis Engine</span>
          </div>
        ...
        <div className="w-full lg:w-auto lg:min-w-[30rem]">
          <div className="flex w-full flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center lg:justify-end gap-2.5 sm:gap-3">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white/[0.03] border border-white/5 rounded-xl text-[9px] sm:text-[10px] font-bold text-white placeholder-zinc-600 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/30 focus-visible:ring-2 focus-visible:ring-indigo-400/45 transition-all uppercase tracking-[0.15em] sm:tracking-widest"
            />
          </div>
          ...
          {!data && !loading && (
            <p className="mt-2 px-1 text-[9px] font-bold uppercase tracking-[0.14em] text-zinc-500 lg:text-right">
              Run analysis first to unlock report download.
            </p>
          )}
        </div>
      </div>
          ...
          <div className="h-full flex-1 flex flex-col items-center justify-center text-center px-4 sm:px-6 py-8 sm:py-10 md:-mt-8">
            <div className="relative mb-6 sm:mb-8">
              ...
              <div className="relative w-28 h-28 sm:w-32 sm:h-32 bg-[#0f111a] rounded-[2.25rem] sm:rounded-[3rem] flex items-center justify-center border border-white/10 shadow-3xl group transition-transform duration-700 hover:scale-110">
                 <Brain size={48} className="text-zinc-800 group-hover:text-indigo-400 transition-colors duration-700 sm:w-14 sm:h-14" strokeWidth={0.5} />
              </div>
            </div>
            <div className="space-y-3">
              <h3 className="text-2xl sm:text-3xl font-black text-white tracking-[0.12em] sm:tracking-widest uppercase">Synthesis Dormant</h3>
        ...
        {loading && (
          <div className="h-full flex-1 flex flex-col items-center justify-center text-center px-4 sm:px-6 py-8 sm:py-10">
            <div className="relative mb-8 sm:mb-10">
              ...
              <Loader2 size={64} className="text-indigo-500/40 animate-spin sm:w-20 sm:h-20" strokeWidth={0.5} />
            </div>
            <div className="space-y-2">
              <h3 className="text-lg sm:text-xl font-black text-white tracking-[0.12em] sm:tracking-widest uppercase animate-pulse">Deploying Logic...</h3>
              <p className="text-zinc-500 text-[9px] font-bold uppercase tracking-[0.2em] sm:tracking-[0.5em]">Cross-Referencing Repository Archives</p>
            </div>
          </div>
        )}
